...
| **Effects** | Three effects on the stereo bus, in series: **Flanger** — modulated feedback comb filter with **Wet**, **Rate** (0.05–8 Hz, log), **Depth**, and **Feedback** knobs; right-channel LFO runs 90° ahead for a wide stereo sweep; **Reverb** (Freeverb) — **Wet**, **Room** (tail length), and **Damp** (HF absorption); **Blackhole** — Eventide-inspired infinite-space reverb with **Wet**, **Gravity** (feedback; 1.0 = infinite tail), **Size** (delay scale 1×–4×), **Damp** (HF rolloff), **Mod** (shimmer LFO depth), and **Pre** (predelay 0–500 ms) |
...
src/
  ...
  flanger.rs       # Modulated feedback comb filter — 512-sample circular buffer, sinusoidal LFO, stereo 90° offset (WASM only)
  reverb.rs        # Freeverb reverb — 8 comb + 4 allpass per channel, stereo (WASM only)
  blackhole.rs     # Eventide Blackhole-inspired reverb — predelay, 8 gravity comb filters, 4 modulated allpass per channel (WASM only)
  keys.rs          # QWERTY → MIDI mapping (terminal only)
  wasm_synth.rs    # wasm-bindgen Synth wrapper — 8-voice polyphony, 2 osc + per-voice filter + reverb + stereo panning per voice, LFO, voice stealing (WASM only)
web/
